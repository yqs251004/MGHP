...
import argparse
from reproduce.train.trainer import GATrainer
from reproduce.datasets.utils import ConversationDataset, make_collate_fn, augment_collate_fn
from reproduce.datasets.get_data import get_repnoise_augment, get_alpaca, get_repnoise
...
parser.add_argument("--name", type=str, default="ga_alpha_"+str(1.0), help="Wandb run name")
args = parser.parse_args()

# wandb.init(project="ga", name="ga_alpha_"+str(args.alpha))
wandb.init(project="ga-iter", name=args.name)

safe_data, unsafe_data = get_repnoise(split='train')
util_data = get_alpaca(split='train')
...
trainer = GATrainer(
    ...
    util_dataloader=alpaca_dataloader,
    epochs=2,
    device=device,
