...
for p in 0.05 0.1 0.15 0.2; do
    save_root="/root/autodl-tmp/outputs/tar_custom_${p}"
    model_dir="${save_root}/final-model"
    trained=0

    # check if the checkpoint exists; if exists, skip training, continue on evaluation
    if [ -d "${model_dir}" ]; then
        echo "Checkpoint for tar_custom_${p} already exists, skipping training."
    else
        python train/train_custom.py \
            --model-path /root/autodl-tmp/outputs/tar/final-model \
            --save-dir "${save_root}" \
            --lr 1e-5 \
            --steps 2000 \
            --harmful_ratio "${p}" \
            ...
            --run_name "tar_custom_${p}_gsm8k_beavertails"
        trained=1
    fi

    # check if the evaluation results already exist; if exists, skip evaluation
    if [ -f "evaluate/saves/tar_custom_${p}_outputs/repnoise_generated.json" ]; then
        echo "Evaluation results for tar_custom_${p} already exists, skipping evaluation."
    else
        python evaluate/generate_repnoise.py \
            --model-path "${model_dir}" \
            --save-dir "evaluate/saves/tar_custom_${p}_outputs"
        python evaluate/gpt_evaluate.py \
            --file-path "evaluate/saves/tar_custom_${p}_outputs/repnoise_generated.json"
    fi
