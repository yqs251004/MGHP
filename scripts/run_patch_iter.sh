...
SAVE_DIR="${SAVE_DIR:-/root/autodl-tmp/outputs/patch_loop_test}"
...
for loop in {1..2}; do
    ...
    GA_DIR="${SAVE_DIR}/patch_${loop}"
    GA_FINAL="${GA_DIR}/final-model"
    SFT_DIR="${SAVE_DIR}/patch_${loop}_mal"
    ...
    if [ -d "$GA_FINAL" ]; then
        echo "Checkpoint for patch_${loop} already exists, skipping GA training."
    else
        if [ "$loop" -eq 1 ]; then
    ...
    if [ -d "$SFT_FINAL" ]; then
        echo "Checkpoint for patch_${loop}_mal already exists, skipping SFT training."
    else
        echo "[RUN] loop=${loop} stage=SFT model_path=${CKPT} save_dir=${SFT_DIR} steps=50 batch_size=${TRAIN_BATCH_SIZE} grad_accum=${GRAD_ACCUM}"
